...
pub fn start(repo: Arc<Repository>) -> Result<()> {
    enable_raw_mode()?;
...
fn push_session_group(items: &mut Vec<HistoryItem>, snaps: &[Snapshot]) {
    ...
    // SAFETY: We just checked that snaps is not empty above
...
fn update_preview(
    ...
    if let Some(ref c) = curr_hash {
        ...
        if let Some(p) = prev_hash {
            ...
            let p_highlighted = highlight_content(&p_content, state, ts_highlighter);
            ...
            for change in diff.iter_all_changes() {
                ...
                let (bg_color, fg_text, sign) = match tag {
                    ChangeTag::Delete => (
                        Some(state.theme.diff_del_bg),
                        Some(state.theme.diff_del_fg),
                        "-",
                    ),
                    ChangeTag::Insert => (
                        Some(state.theme.diff_add_bg),
                        Some(state.theme.diff_add_fg),
                        "+",
                    ),
                    ChangeTag::Equal => (None, None, " "),
                };

                let base_line = match tag {
                    ChangeTag::Delete => p_highlighted.get(change.old_index().unwrap()).cloned(),
                    _ => c_highlighted.get(change.new_index().unwrap()).cloned(),
                };

                if let Some(mut line) = base_line {
                    let is_active_hunk = state.active_hunk_index == Some(current_hunk);
                    let is_selected_hunk = state.selected_hunks.contains(&current_hunk);

                    let mut spans = Vec::new();

                    let marker = match (is_active_hunk, is_selected_hunk) {
                        (true, true) => format!(" >* {} ", sign),
                        (true, false) => format!(" >  {} ", sign),
                        (false, true) => format!("  * {} ", sign),
                        (false, false) => format!("    {} ", sign),
                    };

                    let marker_color = if is_active_hunk {
                        state.theme.accent
                    } else if is_selected_hunk {
                        state.theme.success
                    } else {
                        fg_text.unwrap_or(state.theme.text_dim)
                    };
                    ...
                    spans.push(Span::styled(
                        ...
                        Style::default()
                            .fg(marker_color)
                            .add_modifier(if is_active_hunk {
                                Modifier::BOLD
                            } else {
                                Modifier::empty()
                            }),
        ...
        lines.push(Line::from("Select a file..."));
    }

    state.cached_diff = lines;
    state.last_diff_hash = Some(cache_key);
    state.current_preview_hash = curr_hash.clone();
